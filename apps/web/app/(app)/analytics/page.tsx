...
export default function AnalyticsPage() {
  ...
  // AI Coach — calls the backend's /copilot/interpret-analytics (server-side
  // GROQ_API_KEY). This tab never had any AI wiring before; the section
  // below it ("Local Performance Review") is deliberately rule-based, not AI.
  const [aiInsight, setAiInsight] = useState<string | null>(null);
  const [aiLoading, setAiLoadingState] = useState(false);
  const handleAiInterpret = async () => {
    setAiLoadingState(true);
    try {
      const res = await api.post<{ message: string }>("/copilot/interpret-analytics");
      if (res.success) {
        setAiInsight(res.data.message);
      } else if (res.code === "AI_NOT_CONFIGURED") {
        setAiInsight("AI coaching isn't configured on the server yet. Add GROQ_API_KEY to apps/api/.env.");
      } else if (res.code === "RATE_LIMIT") {
        setAiInsight("AI coaching rate limit reached (20/hour). Try again later.");
      } else {
        setAiInsight(res.error || "AI analysis failed.");
      }
    } catch {
      setAiInsight("AI analysis failed. Check your connection.");
    } finally {

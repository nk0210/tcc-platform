"use client";
/**
 * TCC Owner/Admin Layout
 * Wraps all /owner/* pages with role guard + sidebar navigation.
 */
  ...
  if (!accessChecked) {
    return (
      <div className="min-h-screen bg-[#070710] flex items-center justify-center">
        <p className="text-white/30 text-sm animate-pulse">Checking access...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#070710] flex items-center justify-center">
        <div className="text-center px-6">
          ...
          <button onClick={() => router.push("/")}
            className="bg-white/5 text-white/40 px-4 py-2 rounded-lg text-sm border border-white/10 hover:bg-white/10 transition">
            Return to Dashboard
          </button>
        </div>
      </div>
      ...
      <div className="w-56 shrink-0 bg-black/60 border-r border-red-500/10 flex flex-col">
        ...
        <div className="p-4 border-b border-red-500/10">
          ...
          <div className="mt-2 text-xs text-white/30">
            {user?.handle ?? "Admin"} ·{" "}
            <span className="text-red-400/70 capitalize">{effectiveRole}</span>
          </div>
          {getEffectiveRole(user?.role) !== effectiveRole && (
            <div className="mt-1 text-xs text-amber-400/60 bg-amber-500/10 px-2 py-0.5 rounded-lg">
              DEV role override active
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-0.5 p-2 flex-1 overflow-y-auto">
          {OWNER_NAV.map(item => {
            ...
            const badge =
              ...
              : 0;
            const isCritical =
              item.path === "/owner/reports" && criticalReports > 0;
            ...
            return (
              <button key={item.path}
                ...
                {badge > 0 && (
                  <span className={`text-xs font-bold min-w-[18px] h-[18px] rounded-full flex items-center justify-center px-1 shrink-0 ${

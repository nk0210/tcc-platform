"use client";
/**
 * TCC Analytics Page — Day 5
 *
 * All analytics derived from real paper trading data:
 * - useTradeStore().closedTrades
 * - useTradeStore() account fields
 * - useJournalStore().entries
 * - calculateRiskScore()
 * - TCC_SYMBOL_MAP from central symbol config
 *
 * No fake data. No seed analytics. Empty states when no data.
 * PAPER TRADING ANALYTICS ONLY — not broker-verified.
 */
...
export default function AnalyticsPage() {
  ...
  const riskScore = useMemo(
    ...
    [positions.length, floatingPnl, equity]
  );

  const perf = useMemo(
    () => calculatePerformanceOverview(
      closedTrades as any, balance, equity, floatingPnl, positions as any
    ),
    [closedTrades, balance, equity, floatingPnl, positions]
  );

  const fullEquityCurve = useMemo(
    () => calculateEquityCurve(closedTrades as any),
    [closedTrades]
  );
  ...
  const equityCurve = useMemo(() => {
    ...
    const sorted = [...closedTrades].sort(
      (a, b) => (safeDate(a.closedAt)?.getTime() ?? 0) - (safeDate(b.closedAt)?.getTime() ?? 0)
    );
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex border-b border-white/5 bg-black/20 shrink-0 px-6 items-center">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <span className="text-xs text-green-400/40 bg-green-500/5 border border-green-500/10 px-2 py-0.5 rounded-full">
                Paper Analytics · Local only
              </span>
            </div>
          </div>
              ...
              <div className="flex flex-col gap-6 max-w-5xl">
                <div>
                  ...
                  <p className="text-white/30 text-xs">
                    ...
                    {perf.openPositions > 0 &&
                      ` ${perf.openPositions} position${perf.openPositions > 1 ? "s" : ""} currently open.`}
                  </p>
                </div>
                ...
                {closedTrades.length === 0 ? (
                  <EmptyState
                    ...
                    sub="Close trades from the Dashboard to unlock performance analytics."
                  />
                ) : (
                  <>
                    {/* Stat grid */}
                    <div className="grid grid-cols-4 gap-3">
                      ...
                      <StatCard
                        ...
                        sub="after simulated commission (0.01%)"
                      />
                      <StatCard
                        label="ROI"
                      ...
                      <StatCard
                        ...
                        color={
                          ...
                          : perf.profitFactor >= 1.0 ? "text-amber-400"
                          : "text-red-400"
                        }
                      ...
                      <StatCard
                        ...
                        sub={`${perf.wins} win${perf.wins !== 1 ? "s" : ""} · total $${perf.totalWonAmount}`}
                      />
                      <StatCard
                        label="Avg Loss"
                        value={`-$${perf.avgLoss}`}
                        color="text-red-400"
                        sub={`${perf.losses} loss${perf.losses !== 1 ? "es" : ""} · total $${perf.totalLostAmount}`}
                      />
                      ...
                      <StatCard
                        label="Floating P&L"
                        value={`${perf.floatingPnl >= 0 ? "+" : ""}$${perf.floatingPnl}`}
                        color={perf.floatingPnl >= 0 ? "text-green-400" : "text-red-400"}
                        sub={`${perf.openPositions} open position${perf.openPositions !== 1 ? "s" : ""}`}
                      />
                      <StatCard
                        label="Equity"
                        value={`$${perf.equity.toFixed(2)}`}
                        color={perf.equity >= PAPER_INITIAL_BALANCE ? "text-green-400" : "text-red-400"}
                        sub={`Balance: $${perf.balance.toFixed(2)}`}
                      />
                    </div>
                      ...
                      <div className="glass border border-white/5 rounded-xl p-5">
                        ...
                        <ResponsiveContainer width="100%" height={120}>
                          <BarChart data={monthlyPnl}>
                            ...
                            <Tooltip
                              contentStyle={{ background: "#0a0a0f", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8 }}
                              formatter={(val: any) => [`${Number(val) >= 0 ? "+" : ""}$${Number(val).toFixed(2)}`, "Net P&L"]}
                            />
                            <Bar dataKey="pnl" radius={[4,4,0,0]}>
                              {monthlyPnl.map((m, i) => (
                                <Cell key={i} fill={m.pnl >= 0 ? "#00ff88" : "#ff4466"} />
                              ))}
                            </Bar>
                          </BarChart>
                        </ResponsiveContainer>
                    ...
                    <div className="glass border border-white/5 rounded-xl p-5">
                      <div className="flex items-center justify-between mb-4">
                        <p className="text-white/40 text-xs uppercase tracking-wider">
                          {MONTHS_FULL[calendarMonth]} {calendarYear} — Daily P&L
                        </p>
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            className="text-white/40 hover:text-white px-2 py-1 rounded text-xs border border-white/10 transition">
                            Today
                          </button>
                      ...
                      <div className="grid grid-cols-7 gap-1 mb-2">
                        {DAYS_SHORT.map(d => (
                          <p key={d} className="text-center text-white/20 text-xs py-1">{d}</p>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        ...
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                          ...
                          return (
                            <div
                              key={day}
                              className={`rounded-lg p-1.5 text-center border transition cursor-default ${
                                isToday ? "border-green-400/40" : "border-white/5"
                              } ${dayData ? pnlBg(dayData.pnl) : "bg-white/2"}`}
                              title={dayData ? `${dayData.trades} trade${dayData.trades > 1 ? "s" : ""} · ${dayData.pnl >= 0 ? "+" : ""}$${dayData.pnl.toFixed(2)}` : ""}>
                              <p className={`text-xs ${isToday ? "text-green-400/70" : "text-white/30"}`}>{day}</p>
                              {dayData && (
                                <p className={`text-xs font-bold leading-tight ${pnlColor(dayData.pnl)}`}>
            ...
            {activeTab === "risk" && (
              <div className="flex flex-col gap-6 max-w-5xl">
                <div>
                  ...
                  <p className="text-white/30 text-xs">
                    Risk analysis from paper trading state. Not broker-verified.
                  </p>
                </div>

                {/* Level card + drawdown + streak */}
                <div className="grid grid-cols-3 gap-4">
                  <div className={`glass border rounded-xl p-5 ${
                    riskAnalytics.riskLevel === "EXTREME" ? "border-red-500/30    bg-red-500/5"
                    : riskAnalytics.riskLevel === "HIGH"  ? "border-orange-500/30 bg-orange-500/5"
                    : riskAnalytics.riskLevel === "MEDIUM"? "border-amber-500/30  bg-amber-500/5"
                    :                                        "border-green-500/20  bg-green-500/3"
                  }`}>
                    <p className="text-white/40 text-xs mb-1">Current Risk Level</p>
                    <p className={`text-2xl font-bold ${getRiskColor(riskAnalytics.riskLevel as any)}`}>
                      {riskAnalytics.riskLevel}
                    </p>
                  ...
                  <StatCard
                    label="Max Drawdown"
                    value={`${riskAnalytics.drawdownPercent.toFixed(1)}%`}
                    color={
                      riskAnalytics.drawdownPercent < 5  ? "text-green-400"
                      : riskAnalytics.drawdownPercent < 10 ? "text-amber-400"
                      : "text-red-400"
                    }
                  ...
                  <StatCard
                    ...
                    value={riskAnalytics.maxConsecutiveLosses}
                    color={
                      riskAnalytics.maxConsecutiveLosses >= 4 ? "text-red-400"
                      : riskAnalytics.maxConsecutiveLosses >= 2 ? "text-amber-400"
                      : "text-white"
                    }
                  />
                </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <StatCard
                    label="Open Without SL"
                    value={riskAnalytics.positionsWithoutSL}
                    color={riskAnalytics.positionsWithoutSL > 0 ? "text-red-400" : "text-green-400"}
                    sub={riskAnalytics.positionsWithoutSL > 0 ? "always set a stop loss" : "all positions protected"}
                  />
                </div>
                ...
                {riskAnalytics.maxDrawdownTrade < 0 && (
                  <div className="glass border border-red-500/10 rounded-xl p-5">
                    ...
                    <p className="text-white/30 text-xs mt-1">Largest single-trade loss in paper history.</p>
                  </div>
                )}
                      ...
                      {riskScore.factors.map((f, i) => (
                        <div
                          ...
                          className={`flex items-start gap-3 p-3 rounded-lg ${
                            f.severity === "danger"  ? "bg-red-500/8"
                            : f.severity === "warning" ? "bg-amber-500/8"
                            : "bg-white/3"
                          }`}>
                          ...
                          <div className="flex-1">
                            <p className={`text-xs font-semibold ${
                              f.severity === "danger" ? "text-red-400" : f.severity === "warning" ? "text-amber-400" : "text-white/60"
                            }`}>{f.name}</p>
                ...
                {closedTrades.length === 0 && positions.length === 0 && (
                  <EmptyState
                    message="No active positions or closed trades."
                    sub="Risk analytics populate once you have paper trading activity."
                  />
                )}
              </div>
            )}
              ...
              <div className="flex flex-col gap-6 max-w-5xl">
                ...
                {symbolAnalytics.length === 0 ? (
                  <EmptyState
                    ...
                    sub="Close paper trades to see per-symbol performance breakdown."
                  />
                ) : (
                  <>
                    {/* Table */}
                            ...
                            <tr key={s.symbolId} className="border-b border-white/5 hover:bg-white/2 transition">
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    <p className="text-white font-semibold">{s.displayName}</p>
                                    <p className="text-white/30 capitalize">{s.category}</p>
                                  </div>
                                </div>
                              </td>
                              <td className="px-4 py-3 text-right text-white/60">{s.trades}</td>
                              <td className={`px-4 py-3 text-right font-semibold ${s.winRate >= 50 ? "text-green-400" : "text-red-400"}`}>
                                {s.winRate}%
                              </td>
                              <td className={`px-4 py-3 text-right font-bold ${pnlColor(s.netPnl)}`}>
                                {s.netPnl >= 0 ? "+" : ""}${s.netPnl.toFixed(2)}
                              </td>
                              <td className={`px-4 py-3 text-right ${pnlColor(s.avgPnl)}`}>
                    ...
                    {symbolAnalytics.length > 1 && (
                      <div className="glass border border-white/5 rounded-xl p-5">
                        ...
                        <ResponsiveContainer width="100%" height={Math.max(100, symbolAnalytics.length * 30)}>
                          <BarChart data={symbolAnalytics} layout="vertical">
                            ...
                            <Bar dataKey="netPnl" radius={[0,4,4,0]}>
                              {symbolAnalytics.map((s, i) => (
                                <Cell key={i} fill={s.netPnl >= 0 ? "#00ff88" : "#ff4466"} />
                              ))}
                            </Bar>
                          </BarChart>
                        </ResponsiveContainer>
            ...
            {activeTab === "sessions" && (
              <div className="flex flex-col gap-6 max-w-5xl">
                <div>
                  ...
                  <p className="text-white/30 text-xs">
                    Derived from journal entries. Tag strategies in your journal to unlock strategy analytics.
                  </p>
                </div>
                ...
                <div className="glass border border-white/5 rounded-xl p-5">
                  ...
                  {sessionAnalytics.length === 0 ? (
                    <EmptyState
                      message="No journal data with session tags yet."
                      sub="Close trades — journal entries are auto-created with session detection."
                    />
                  ) : (
                    <>
                        ...
                        {sessionAnalytics.map(s => (
                          <div key={s.session} className="flex items-center gap-4">
                            ...
                            <div className="flex-1 bg-white/5 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${s.winRate >= 50 ? "bg-green-400" : "bg-red-400"}`}
                                style={{ width: `${Math.min(s.winRate, 100)}%` }}
                              />
                            </div>
                            <span className={`text-xs font-semibold w-20 text-right ${pnlColor(s.netPnl)}`}>
                              {s.netPnl >= 0 ? "+" : ""}${s.netPnl.toFixed(2)}
                            </span>
                            <span className="text-white/30 text-xs w-16 text-right">{s.winRate}% WR</span>
                            <span className="text-white/20 text-xs w-12 text-right">{s.trades}t</span>
                          </div>
                        ))}
                      </div>
                ...
                <div className="glass border border-white/5 rounded-xl p-5">
                  ...
                  {strategyAnalytics.length === 0 ? (
                    <EmptyState
                      message="No strategy data yet."
                      sub="Tag strategies in your journal entries to unlock this section."
                    />
                  ) : (
                        ...
                        <tbody>
                          {strategyAnalytics.map(s => (
                            <tr key={s.strategy} className="border-b border-white/5 hover:bg-white/2 transition">
                              ...
                              <td className="px-4 py-3 text-right text-white/60">{s.trades}</td>
                              <td className={`px-4 py-3 text-right ${s.winRate >= 50 ? "text-green-400" : "text-red-400"}`}>
                                {s.winRate}%
                              </td>
                              <td className={`px-4 py-3 text-right font-bold ${pnlColor(s.netPnl)}`}>
                                {s.netPnl >= 0 ? "+" : ""}${s.netPnl.toFixed(2)}
            ...
            {activeTab === "behavior" && (
              <div className="flex flex-col gap-6 max-w-5xl">
                <div>
                  ...
                  <p className="text-white/30 text-xs">
                    Patterns from your journal — emotion, discipline, entry quality. Fill in journal fields for richer data.
                  </p>
                </div>

                {behaviorAnalytics.totalClosedEntries === 0 ? (
                  <EmptyState
                    message="No journal data yet."
                    sub="Behavior analytics require closed paper trades with journal entries."
                  />
                ) : (
                  <>
                    {/* Summary row */}
                    <div className="grid grid-cols-4 gap-3">
                      ...
                      <StatCard
                        ...
                        value={behaviorAnalytics.withPlanDataCount > 0
                          ? `${behaviorAnalytics.followedPlanPercent}%`
                          : "No data"}
                        color={
                          behaviorAnalytics.followedPlanPercent >= 70 ? "text-green-400"
                          : behaviorAnalytics.followedPlanPercent >= 50 ? "text-amber-400"
                          : "text-red-400"
                        }
                        sub={behaviorAnalytics.withPlanDataCount > 0
                          ? `${behaviorAnalytics.didNotFollowPlanCount} deviations`
                          : "Mark plan adherence in journal"}
                      />
                      <StatCard
                        label="Avg Confidence"
                        value={`${behaviorAnalytics.avgConfidence}/10`}
                        color={behaviorAnalytics.avgConfidence >= 7 ? "text-green-400" : "text-amber-400"}
                      />
                      <StatCard
                        label="Avg Stress"
                        value={`${behaviorAnalytics.avgStress}/10`}
                        color={
                          behaviorAnalytics.avgStress <= 4 ? "text-green-400"
                          : behaviorAnalytics.avgStress <= 6 ? "text-amber-400"
                          : "text-red-400"
                        }
                      />
                    </div>
                    ...
                    <div className="glass border border-white/5 rounded-xl p-5">
                      ...
                      <div className="flex gap-3">
                        {[
                          ...
                          <div key={item.label} className={`flex-1 ${item.bg} border border-white/5 rounded-xl p-4 text-center`}>
                            <p className={`text-2xl font-bold ${item.color}`}>{item.count}</p>
                            <p className="text-white/30 text-xs mt-1 leading-tight">{item.label}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                          ...
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              {behaviorAnalytics.emotionBreakdown.map(e => (
                                <tr key={e.emotion} className="border-b border-white/5 hover:bg-white/2 transition">
                                  <td className="px-4 py-3 text-white capitalize">{e.emotion}</td>
                                  <td className="px-4 py-3 text-right text-white/60">{e.trades}</td>
                                  <td className={`px-4 py-3 text-right ${e.winRate >= 50 ? "text-green-400" : "text-red-400"}`}>
                                    {e.winRate}%
                                  </td>
                                  <td className={`px-4 py-3 text-right font-bold ${pnlColor(e.netPnl)}`}>
            ...
            {activeTab === "intelligence" && (
              <div className="flex flex-col gap-6 max-w-5xl">
                <div>
                  ...
                  <p className="text-white/30 text-xs">
                    Rule-based discipline score, funded challenge readiness, and local performance review.
                    Paper trading data only — not broker-verified.
                  </p>
                </div>
                  ...
                  {!disciplineScore.hasEnoughData ? (
                    <div className="flex flex-col gap-2">
                      <p className="text-white/50 text-sm">
                        Discipline score unlocks after 5 closed paper trades.
                      </p>
                      <div className="w-full bg-white/5 rounded-full h-2 mt-2">
                        <div
                          className="bg-green-500/40 h-2 rounded-full transition-all"
                          style={{ width: `${Math.min((closedTrades.length / 5) * 100, 100)}%` }}
                        />
                      </div>
                      <p className="text-white/20 text-xs">{closedTrades.length}/5 closed trades</p>
                    </div>
                  ) : (
                    <div className="flex gap-8">
                      {/* Score dial */}
                      <div className="flex flex-col items-center justify-center w-36 shrink-0 gap-2">
                        <div className={`text-6xl font-black leading-none ${
                          disciplineScore.total >= 85 ? "text-green-400"
                          : disciplineScore.total >= 70 ? "text-blue-400"
                          : disciplineScore.total >= 55 ? "text-amber-400"
                          : disciplineScore.total >= 40 ? "text-orange-400"
                          : "text-red-400"
                        }`}>{disciplineScore.total}</div>
                        <p className="text-white/30 text-xs">/100</p>
                        <span className={`text-sm font-bold px-3 py-1 rounded-full ${
                          disciplineScore.grade === "A" ? "text-green-400 bg-green-500/15"
                          : disciplineScore.grade === "B" ? "text-blue-400 bg-blue-500/15"
                          : disciplineScore.grade === "C" ? "text-amber-400 bg-amber-500/15"
                          : disciplineScore.grade === "D" ? "text-orange-400 bg-orange-500/15"
                          : "text-red-400 bg-red-500/15"
                        }`}>Grade {disciplineScore.grade}</span>
                        <div className="w-full bg-white/10 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all ${
                              disciplineScore.total >= 70 ? "bg-green-400"
                              : disciplineScore.total >= 50 ? "bg-amber-400"
                              : "bg-red-400"
                            }`}
                            style={{ width: `${disciplineScore.total}%` }}
                          />
                        </div>
                      </div>
                        ...
                        {disciplineScore.components.map(c => (
                          <div key={c.name}>
                            <div className="flex justify-between mb-0.5">
                              ...
                              <span className="text-white/40 text-xs">{c.score}/{c.maxScore} pts</span>
                            </div>
                            <div className="w-full bg-white/5 rounded-full h-1.5 mb-0.5">
                              <div
                                className={`h-1.5 rounded-full transition-all ${
                                  c.score >= c.maxScore * 0.7 ? "bg-green-400"
                                  : c.score >= c.maxScore * 0.4 ? "bg-amber-400"
                                  : "bg-red-400"
                                }`}
                ...
                <div className="glass border border-white/5 rounded-xl p-5">
                  ...
                  <div className={`flex items-center gap-4 mb-5 p-4 rounded-xl border ${
                    fundedReadiness.level === "strong"           ? "border-green-500/30  bg-green-500/5"
                    : fundedReadiness.level === "moderate"       ? "border-amber-500/30  bg-amber-500/5"
                    : fundedReadiness.level === "building"       ? "border-orange-500/20 bg-orange-500/3"
                    :                                              "border-white/10       bg-white/2"
                  }`}>
                    <span className="text-3xl">
                      {fundedReadiness.level === "strong"             ? "🟢"
                       : fundedReadiness.level === "moderate"         ? "🟡"
                       : fundedReadiness.level === "building"         ? "🔵"
                       :                                                "⚪"}
                    </span>
                    <div>
                      <p className={`font-bold text-base ${
                        fundedReadiness.level === "strong"   ? "text-green-400"
                        : fundedReadiness.level === "moderate" ? "text-amber-400"
                        : "text-white/70"
                      }`}>{fundedReadiness.label}</p>
                      <p className="text-white/40 text-xs mt-0.5 leading-relaxed">
                        {fundedReadiness.description}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex flex-col gap-2.5 mb-5">
                      {fundedReadiness.components.map(c => (
                        <div key={c.name} className="flex items-center gap-3">
                          ...
                          <span className="text-white/60 text-xs w-44 shrink-0">{c.name}</span>
                ...
                <div className="glass border border-white/5 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-white/20 text-xs mt-0.5">
                        Rule-based insights from your paper trading data.
                        Not AI — not financial advice.
                      </p>
                    </div>
                    <span className="text-xs bg-white/5 text-white/30 border border-white/10 px-2 py-0.5 rounded-full">
                      Rule-based · Local only
                    </span>
                  </div>
                  ...
                  {insights.length === 0 ? (
                    <EmptyState
                      ...
                      sub="Close more paper trades to generate a meaningful local performance review."
                    />
                  ) : (
                      ...
                      {insights.map((insight, i) => (
                        <div
                          ...
                          className={`flex items-start gap-3 p-3 rounded-xl border ${
                            insight.type === "positive" ? "bg-green-500/5  border-green-500/15"
                          ...
                          <p className={`text-sm leading-relaxed ${
                            insight.type === "positive" ? "text-green-400/90"
                            : insight.type === "warning" ? "text-amber-400/90"
                            : "text-white/60"
                          }`}>{insight.text}</p>
                        </div>
                      ))}

 ...
 * All calculations use lib/trading/calculations.ts (internal paper model).
 * NOT broker-accurate. Paper Mode only.
 */
...
export interface PaperPosition {
  ...
  openedAt: string; // ISO string — JSON-safe
}
...
/** Events emitted by the trade store — consumed by hooks/components */
...
interface TradeStore {
  ...
  // Event queue — consumed by useSystemNotifications or components
  events: TradeEvent[];
  ...
  // Legacy compatibility (Topbar uses totalNetPnl)
  totalNetPnl: number;
}
  ...
  persist(
    (set, get) => ({
      ...
      closePosition: (positionId, reason = "manual") => {
        const { positions, balance, leverage, closedTrades, events } = get();
      ...
      updatePrices: (symbol, price) => {
        ...
        const { positions, balance, events } = get();
        ...
        const updatedPositions = positions.map(p => {
          ...
          // Check SL/TP before updating price
          if (isStopLossTriggered(p.side, price, p.sl)) {
            slTpToClose.push({ id: p.id, reason: "stop_loss" });
            return { ...p, currentPrice: price }; // Update price before closing
          }
          ...
          const grossPnl = calcGrossPnl(p.symbol, p.side, p.lotSize, p.entryPrice, price);
          const netPnl = calcNetPnl(p.symbol, p.side, p.lotSize, p.entryPrice, price);
        ...
        // Process SL/TP closures after price update is committed
        slTpToClose.forEach(({ id, reason }) => {
          get().closePosition(id, reason);
        });
      },
      ...
      setLeverage: (leverage) => {
        ...
        const account = recalcAccount(updatedPositions, balance);
        set({ leverage, positions: updatedPositions, ...account, totalNetPnl: account.floatingPnl });
      },
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // events are NOT persisted — ephemeral
      }),

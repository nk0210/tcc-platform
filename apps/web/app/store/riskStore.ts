...
export function calculateRiskScore(): RiskScore {
  ...
  const safeBalance = balance > 0 ? balance : 1;
  ...
  if (positions.length >= 5) {
    factors.push({
      name: "Overtrading",
      score: 20,
      description: "5+ open positions simultaneously",
    });
  ...
  const floatingPct = (Math.abs(Math.min(floatingPnl, 0)) / safeBalance) * 100;

  if (floatingPct >= 5) {
    factors.push({
      name: "Large floating loss",
      score: 25,
      description: `${floatingPct.toFixed(1)}% floating loss`,
    });
  } else if (floatingPct >= 2) {
    factors.push({
      name: "Floating loss",
      score: 10,
      description: `${floatingPct.toFixed(1)}% floating loss`,
  ...
  if (marginLevel > 0 && marginLevel < 150) {
    factors.push({
      name: "Critical margin",
      score: 30,
      description: `Margin level at ${marginLevel}%`,
    });
  ...
  if (noSL > 0) {
    factors.push({
      ...
      score: noSL * 10,
      description: `${noSL} position(s) without SL`,
    });
  }

  // Factor 5: Equity drawdown
  const drawdown = ((balance - equity) / safeBalance) * 100;

  if (drawdown >= 10) {
    factors.push({
      name: "High drawdown",
      score: 20,
      description: `${drawdown.toFixed(1)}% equity drawdown`,
    });
  } else if (drawdown >= 5) {
    factors.push({
  ...
  if (total >= 75) {
    level = "EXTREME";
    recommendation =
      "EXTREME RISK: Stop trading immediately. Close positions and review your risk management.";
  } else if (total >= 50) {
    level = "HIGH";
    recommendation =
      "HIGH RISK: Reduce exposure. Consider closing losing positions and tightening SL.";
  } else if (total >= 25) {
    level = "MEDIUM";
    recommendation =
      "MEDIUM RISK: Monitor closely. Avoid opening new positions until risk reduces.";
  }
